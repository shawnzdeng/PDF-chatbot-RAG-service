...
Write-Host "   # Update OpenAI API Key:" -ForegroundColor White
Write-Host "   kubectl create secret generic rag-secrets --from-literal=openai-api-key=YOUR_OPENAI_API_KEY --from-literal=qdrant-api-key=YOUR_QDRANT_API_KEY -n rag-system --dry-run=client -o yaml | kubectl apply -f -" -ForegroundColor White
Write-Host ""
